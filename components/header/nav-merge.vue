<template>
  <nav class="w-full flex flex-col">
    <div class="w-full flex items-center justify-between mx-auto py-4 px-2 md:px-24">
      <!-- logo 部分 -->
      <HeaderLogoSvg />
      <!-- 语言设置 / 拓展菜单 -->
          ...
          <template #label>
            <div class="group flex items-center border rounded-full py-1 pl-4 pr-2 text-sm">
              ...
              <IconTriangleDown
                ...
                size="w-6 h-4"
                class="origin-center transform group-hover:rotate-180 transition-normal"
              />
            </div>
          </template>
          <!-- 语言选项 -->
      ...
      <div
        ...
        class="
        w-64 h-screen md:w-auto md:h-auto
        fixed md:relative right-0 inset-y-0 md:m-0
        flex-grow md:block
        md:bg-transparent shadow-lg md:shadow-none
        transition-normal
        z-30
      "
      >
        <div class="flex flex-col md:flex-row justify-end items-center text-sm">
          <!-- 关闭按钮 -->
          <section class="flex justify-between md:hidden m-4 pb-2 text-right border-b" :class="`border-${theme}-mainBd`">
          ...
          <!-- 操作部分 -->
          <a v-if="!extensionInstalled" href="https://chrome.google.com/webstore/detail/qiwallet/cjfeohjffkdehdblcolicjhhnbphcmna" target="_blank" :class="`bg-transparent md:bg-${theme}-assist-100`" class="cursor-pointer leading-8 md:leading-normal text-center mr-0 md:mr-4 py-4 md:py-1 px-6 rounded-full hover:opacity-75 transition-normal">
            {{ $t('nav.install') }}
          </a>
          <div
          ...
          <div v-else class="mr-0 md:mr-4">
            <div
              ...
              class="cursor-pointer leading-10 md:leading-normal text-center p-4 md:p-px rounded-full hover:opacity-75 transition-normal"
              @click="showTxs = true"
            >
              <template v-if="pendings === 0">
                <span class="inline-block md:py-1 pl-3 pr-2">{{ account.balance }} QTUM</span>
                <span :class="`md:bg-${theme}-assist-100`" class="h-full inline-block md:py-1 px-3 rounded-full">{{ account.address.slice(0, 4) }}...{{ account.address.slice(-4) }}</span>
              </template>
          ...
          <a :href="`https://${domain}`" target="_blank" class="text-center leading-8 md:leading-normal py-4 md:mx-6 transition-normal">
            {{ $t('nav.browser') }}
          </a>
    ...
    <Modal v-model="showTxs" :title="$t('nav.accountModal.title')">
      <div v-if="account" class="">
        <!-- info -->
        <div class="w-full mx-auto py-4 px-4 mb-4 rounded-lg" :class="`bg-${theme}-main-400`">
          <div class="flex mb-4">
          ...
          <div class="flex justify-between pl-8">
            <button v-clipboard:copy="account.address" v-clipboard:success="copyFinish" class="flex items-center text-xs hover:opacity-75 border px-2 py-px rounded-full" :class="`text-${theme}-inverse-500 border-${theme}-inverse-500`">
              <img src="@/assets/icons/copy.svg" alt="" class="w-3 mr-1">
              <span>{{ copyState ? $t('nav.accountModal.copied') : $t('nav.accountModal.copy') }}</span>
            </button>
            <a :href="`https://${domain}/address/${account.address}/`" class="flex items-center text-xs hover:opacity-75 border px-2 py-px rounded-full" :class="`text-${theme}-inverse-500 border-${theme}-inverse-500`" target="_blank">
        ...
        <div class="flex flex-col overflow-hidden py-2 px-4 rounded-lg" style="height: 20rem;" :class="`bg-${theme}-main-400`">
          ...
          <div v-if="filteredTxs.length === 0" class="flex-grow flex flex-col items-center mt-12">
...
<script>
import { defineComponent, ref, computed, useContext, onMounted, getCurrentInstance } from '@nuxtjs/composition-api'
...
export default defineComponent({
  setup () {
    ...
    const domain = computed(() => DOMAIN[account.value?.network || NETWORK.MainNet])
    const filteredTxs = computed(() => state.swap.txs.filter(tx => {
      return tx.address === account.value.address
    }))
    const pendings = computed(() => filteredTxs.value.filter(t => t.raw.confirmations === 0).length)
    const connect = async () => {
      if (connecting.value) {
        return
      }
