...
export default class Token {
  constructor ({ name, decimals, symbol, address, chainId, icon = '/icons/default.svg', totalSupply, balanceSatoshi, imported = false }) {
    this.name = name
  ...
  eq (token) {
    return (token?.isQTUM && this.isQTUM) || token === this || isSameToken(this, token)
  }
  ...
  async getBalance (forceUpdate = false) {
    ...
    if (this.isQTUM) {
      return BigNumber(qrypto.account?.balance || 0).times(10 ** Token.QTUM.decimals)
    } else {
  ...
  static wrapToken (token) {
    ...
    return token instanceof Token ? token.isQTUM ? Token.WQTUM[chainId] : token : undefined
  }
}
...
Token.WQTUM = Object.fromEntries(WQTUM.map(wQtum => [wQtum.chainId, new Token({
  name: 'Wrapped Qtum',
  symbol: 'WQTUM',
  decimals: 8,
  icon: '/icons/qtum.svg',
