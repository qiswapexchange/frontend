...
export function sortTokenAmounts (tokenAmount0, tokenAmount1) {
  const [token0] = sortTokens(tokenAmount0.wrappedToken, tokenAmount1.wrappedToken)
  return tokenAmount0.wrappedToken.eq(token0) ? [tokenAmount0, tokenAmount1] : [tokenAmount1, tokenAmount0]
}
...
export default class TokenAmount {
  ...
  set amount (amount) {
    const amountSatoshi = BigNumber(amount).times(10 ** this.decimals).dp(0, BigNumber.ROUND_DOWN)
    this.amountSatoshi = amountSatoshi
  }
