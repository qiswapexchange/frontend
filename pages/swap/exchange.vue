<template>
  <div class="">
    ...
    >
      1 {{ tokenAmount0.symbol }} = {{ swap.ratio.toSd(6) }}
      {{ tokenAmount1.symbol }}
      <div class="flex items-center justify-center text-sm">
        <div
          v-for="(token, i) in route.path"
          :key="i"
    ...
    <div
      ...
      "
      class="rounded-lg mb-4 px-6 py-4 text-sm"
    ...
    >
      <div class="flex justify-between mb-4">
        <span>
          ...
          <img
            v-tooltip.top="{
              content: $t('swap.swapInfo.impact.tip'),
              class: `tooltip-${theme}`,
            }"
        ...
        <span>
          {{
            swap.priceImpact.lt(0.0001)
              ? '&lt;0.01'
              : swap.priceImpact.toPercent()
          }}%
        </span>
      </div>
      <div class="flex justify-between mb-4">
        <span>
          {{
            swap.swapType === SWAP_EXACT_INPUT
              ? $t('swap.swapInfo.minimum.label')
              : $t('swap.swapInfo.maximum.label')
          }}
          <img
            v-tooltip.right="{
              content:
                swap.swapType === SWAP_EXACT_INPUT
                  ? $t('swap.swapInfo.minimum.tip')
                  : $t('swap.swapInfo.maximum.tip'),
              class: `tooltip-${theme}`,
            }"
            src="@/assets/icons/tip.svg"
            alt=""
        ...
        <span v-if="swap.swapType === SWAP_EXACT_INPUT">
          {{ tokenAmount1.getAmount(swap.minimumAmountOut) }}
          {{ tokenAmount1.symbol }}
        </span>
        <span v-else>
          {{ tokenAmount0.getAmount(swap.maximumAmountIn) }}
          {{ tokenAmount0.symbol }}
        </span>
      </div>
      <div class="flex justify-between">
        <span>
          ...
          <img
            v-tooltip.bottom="{
              content: $t('swap.swapInfo.fee.tip'),
              class: `tooltip-${theme}`,
            }"
            src="@/assets/icons/tip.svg"
            alt=""
        ...
        <span
          >{{ swap.fee.times(tokenAmount0.amount) }}
          {{ tokenAmount0.symbol }}</span
        >
      </div>
    </div>
    <!-- 按钮 -->
